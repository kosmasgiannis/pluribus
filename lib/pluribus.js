...
function execute(processName, config) {
    ...
    if (typeof config.msgHandler === "undefined") {
        config.msgHandler = {};
    }
    ...
    function log() {
        ...
        args.unshift(cluster.isPrimary ? "Pluribus Master" : "Pluribus Worker");
    ...
    function messageFromWorker(msg) {
        ...
        // Handle other messages with custom function, if specified.
        if (typeof config.msgHandler.master === "function") {
            config.msgHandler.master(msg);
        }
    }
    ...
    function createKillHandler(signal) {
        ...
        function killMaster() {
            if (typeof config.killMaster === "function") {
              config.killMaster(function() {
    ...
    if (cluster.isPrimary) {
        ...
        cluster.on(exitEvent, function (worker) {
            ...
            if (typeof config.workerOnExit === "function") {
              config.workerOnExit(worker);
            }
        ...
        // Trigger action when worker becomes online
        cluster.on('online', function (worker) {
          if (typeof config.workerOnOnline === "function") {
            config.workerOnOnline(worker);
          }
        });
        ...
        // Handle messages comming from master
        if (typeof config.msgHandler.worker === "function") {
          process.on('message', config.msgHandler.worker);
        }
